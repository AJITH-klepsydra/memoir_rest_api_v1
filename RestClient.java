...
public class MemoirRestClient{
    public static void main(String[] args) {
        ...
        System.out.println("\nFETCHING ALL NOTES...");
        System.out.println(RestClient.get_all_notes(auth_token));
        ...
        System.out.println("\nFCREATING A  NOTE...");
        System.out.println(RestClient.create_note("title","description","2020-12-25 22:25:14",auth_token));
        System.out.println("\nGET OR DELETE A NOTE...");
        Note note = RestClient.get_or_delete_note(3,"GET",auth_token);
        System.out.println(note.id);
        System.out.println("\nMAKING MODIFICATIONS TO A NOTE...");
        System.out.println(RestClient.put_note(note,auth_token));
    }
}
...
//NOTES
    //get all the notes
    public static ArrayList<Note> get_all_notes(String token){
        ...
        try {
            ...
            list = JsonParserer.parsenotes(result);
            conn.disconnect();
      } catch (MalformedURLException e) {
        e.printStackTrace();
      } catch (IOException e) {
    ...
    //Create a new note
    public static String create_note(String title,String description,String end_date,String token){
        String status = "token is not available";
        try {
            URL url = new URL("https://memoir-api-v1.herokuapp.com/notes/?format=json");
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
    ...
    public static Note get_or_delete_note(int val,String method,String token) {
        Note note = new Note();
        try {
    ...
    public static String put_note(Note note,String token){
...
class JsonParserer{
    ...
    public static ArrayList<Note> parsenotes(String data){
        ArrayList<Note> s = new ArrayList<Note>();
        if (data.startsWith("[{")){
            String[] arrobj = data.split("},");
            for(String obj: arrobj){
                ArrayList<String> args = new ArrayList<String>();
...
class Note{
    ...
    public  Note(ArrayList<String> args ){
        this.id = Integer.parseInt(args.get(0));
        this.title = args.get(1);
        this.published_date = args.get(2);
        this.description = args.get(3);
